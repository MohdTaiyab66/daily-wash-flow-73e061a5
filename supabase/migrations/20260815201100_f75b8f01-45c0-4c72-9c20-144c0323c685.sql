-- Fix for "column reference broadcast_id is ambiguous" in ON CONFLICT
-- This occurs when a table column name matches a column name in the source SELECT.
-- We explicitly qualify the target columns in the ON CONFLICT clause.
...
BEGIN
  ...
  -- The fix: explicitly refer to target table columns to avoid ambiguity
