-- 1. Redefine the function to correctly use the 'partners' table
...
BEGIN
  ...
  IF v_booking.payment_status != 'paid' OR v_booking.partner_id IS NOT NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Booking is not eligible for assignment (Status: ' || v_booking.payment_status || ')');
  END IF;

  -- 2. Validate partner using the AUTHORITATIVE table: public.partners
  -- Note: partner_profiles.user_id is now partners.id
  ...
  -- 6. Create Partner Notification (Wrapped to prevent failure from blocking assignment)
  BEGIN
    INSERT INTO public.partner_notifications (
      ...
      'You have been assigned to a new service for ' || (SELECT full_name FROM customers WHERE id = v_booking.user_id),
      'assignment_new',
  ...
  -- 8. Audit Record (Optional check for table existence)
...
-- Ensure the 2-argument version is also correct if it exists
CREATE OR REPLACE FUNCTION public.admin_assign_partner_to_booking(
  p_booking_id uuid,
  p_partner_id uuid
)
