  ...
  FOR z IN SELECT * FROM coverage_zones ORDER BY priority DESC, name LOOP
    ...
    SELECT COUNT(*) INTO ds_customers FROM subscriptions s
      ...
      WHERE s.status='active'
        AND (c.latitude IS NULL OR z.bbox_min_lat IS NULL
             OR (c.latitude BETWEEN z.bbox_min_lat AND z.bbox_max_lat
                 AND c.longitude BETWEEN z.bbox_min_lng AND z.bbox_max_lng));
