...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    THEN
      ...
      -- Customer ETA notifications intentionally removed.
      -- Customers see only their promised service window; route reordering
      -- that still fits the window must remain invisible to them.
    END IF;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'changed',v_changed,'eta_notifications',0
  );
END $function$;