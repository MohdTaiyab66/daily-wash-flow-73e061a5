...
BEGIN
  ...
  -- Marking as completed fires triggers that:
  --  * insert wallet_ledger 'earning' row (tg_credit_completed_service)
  --  * insert customer_notifications 'service_completed' row (tg_notify_customer_service_status)
  ...
  -- Ensure earnings summary row exists (trigger only touches wallet_ledger)
  ...
  -- Read wallet balance after trigger-posted earning
  SELECT COALESCE(SUM(CASE WHEN entry_type IN ('earning','bonus') THEN amount
                           WHEN entry_type IN ('deduction','penalty','payout') THEN -amount ELSE 0 END),0)
    INTO v_wallet_balance FROM public.wallet_ledger WHERE partner_id = v_partner;

  -- Resolve customer user id, partner name, photos, complaint window
  ...
  -- Enrich the trigger-created customer notification with partner details, photos, and complaint window
  ...
  -- Partner notification: single earnings row per service
