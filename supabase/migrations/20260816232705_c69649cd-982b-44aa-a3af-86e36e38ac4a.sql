...
AS $function$
 ...
 BEGIN
   EXECUTE 'INSERT INTO public.marketplace_offers (broadcast_id, partner_id, incentive, response, round)
   SELECT
     mb.id,
     $1,
   ...
   JOIN public.partners p ON p.id = $1
   WHERE mb.status = ''open''
     AND mb.winning_partner_id IS DISTINCT FROM $1
