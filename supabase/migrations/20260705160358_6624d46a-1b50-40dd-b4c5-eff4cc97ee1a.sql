...
BEGIN
  ...
  ELSE
    ...
    WHERE cp.user_id = v_booking.user_id
    LIMIT 1;

    SELECT concat_ws(', ', address_line, area, pincode) INTO v_addr
    FROM public.customer_addresses
    WHERE id = v_booking.address_id;
    ...
    INSERT INTO public.service_leads(
      ...
      'new', jsonb_build_object('source', 'payment', 'vehicle_id', v_booking.vehicle_id)
    )
    ...
    VALUES('service_lead', 'New service lead: ' || v_booking.service_name,
           COALESCE(v_cust_name, 'Customer') || ' booked ' || v_booking.service_name || ' for ' || COALESCE(v_veh_label, 'vehicle') || ' (₹' || v_booking.total_amount || ')',
           'info', jsonb_build_object('lead_id', v_lead, 'booking_id', p_booking_id, 'vehicle_id', v_booking.vehicle_id));
...
REVOKE EXECUTE ON FUNCTION public.activate_paid_booking(uuid, text, text, text, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.activate_paid_booking(uuid, text, text, text, jsonb) TO authenticated, service_role;

UPDATE public.service_leads l
SET vehicle_label = concat_ws(' ', cv.make, cv.model, NULLIF(cv.registration_number, ''))
FROM public.customer_vehicles cv
WHERE cv.id = l.vehicle_id
  AND l.vehicle_label IS DISTINCT FROM concat_ws(' ', cv.make, cv.model, NULLIF(cv.registration_number, ''));