DROP FUNCTION IF EXISTS public.get_partner_open_offers(uuid);

CREATE OR REPLACE FUNCTION public.get_partner_open_offers(p_pid uuid)
 RETURNS TABLE(
   id uuid, 
   offer_broadcast_id uuid, 
   offer_partner_id uuid, 
   offer_round integer, 
   offer_incentive numeric, 
...
AS $function$
 BEGIN
   ...
   JOIN public.partners p ON p.id = p_pid
   WHERE mb.status = 'open'
     AND mb.winning_partner_id IS DISTINCT FROM p_pid
   ...
   SELECT
     o.id, 
     o.broadcast_id as offer_broadcast_id, 
     o.partner_id as offer_partner_id, 
     o.round as offer_round, 
     o.incentive as offer_incentive,
