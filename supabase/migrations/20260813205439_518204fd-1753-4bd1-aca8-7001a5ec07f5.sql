...
BEGIN
    ...
    -- 2. Get current round radius (Round 1 = index 0)
    ...
    WITH partner_metrics AS (
        ...
        WHERE (p.status = 'active' OR p.status = 'pending_verification') -- Allow pending_verification for testing if they are online
          AND p.availability = 'online'
          AND (
            p.home_zone_id = v_broadcast.service_area_id
            OR 
            (v_round_radius > 0 AND false) -- Radius check placeholder
          )
    )
    SELECT 
        pid,
