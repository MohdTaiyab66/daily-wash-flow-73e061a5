...
BEGIN
  ...
  IF v_is_dirty THEN
    -- dirty_vehicle is a workflow outcome, not a member of the unavailable_reason enum.
    -- Store the service outcome as "other" and keep the dirty reason/details in notes + dirty_vehicle_reports.
    v_service_reason := 'other'::public.unavailable_reason;
  ELSE
    IF p_reason NOT IN (
  ...
  IF v_service.partner_id <> v_partner THEN RAISE EXCEPTION 'Service not assigned to this partner'; END IF;
  ...
  -- Unavailable and dirty both count as a resolved stop and get the configured partner credit.
  IF v_credit > 0 THEN
