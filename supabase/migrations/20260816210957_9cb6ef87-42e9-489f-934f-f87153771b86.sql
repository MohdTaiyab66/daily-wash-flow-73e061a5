-- 1. Extend the assignments table with sub_status for explicit tracking
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'assignments' AND column_name = 'sub_status') THEN
    ALTER TABLE public.assignments ADD COLUMN sub_status text DEFAULT 'customers_available';
  END IF;
END $$;

-- 2. Update existing functions to set correct sub_status on creation
...
BEGIN
  ...
  INSERT INTO public.assignments (
    partner_id, area, target_cars, status, sub_status, rate_per_car, 
    estimated_earnings, estimated_hours, estimated_distance_km, 
    search_radius_km, scheduled_date, duration_days, start_date, 
    end_date, working_days, expected_start_time, total_earnings, 
    original_duration_days
  )
  VALUES (
    v_partner, v_home_area, p_cars, 'active', 
    CASE WHEN v_found > 0 THEN 'customers_available' ELSE 'waiting_for_customers' END,
    v_rate, p_cars * v_rate,
    GREATEST(1, ceil(p_cars::numeric / 5)), v_total_d, v_radius,
...
-- 3. Trigger to automatically update sub_status when work arrives
CREATE OR REPLACE FUNCTION public.update_assignment_sub_status_on_service()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.assignment_id IS NOT NULL THEN
    UPDATE public.assignments
    SET sub_status = 'customers_available'
    WHERE id = NEW.assignment_id AND sub_status = 'waiting_for_customers';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_update_assignment_sub_status ON public.services;
CREATE TRIGGER trg_update_assignment_sub_status
AFTER INSERT ON public.services
FOR EACH ROW
EXECUTE FUNCTION public.update_assignment_sub_status_on_service();

-- 4. View for Admin visibility
...
WHERE a.status = 'active' 
  AND a.sub_status = 'waiting_for_customers';
...
-- 5. Grants for sub_status visibility
