GRANT SELECT, INSERT, UPDATE ON public.unavailability_reports TO authenticated;
...
DROP POLICY IF EXISTS "unavailability reports: partner create own" ON public.unavailability_reports;
DROP POLICY IF EXISTS "unavailability reports: partner admin customer read" ON public.unavailability_reports;
DROP POLICY IF EXISTS "unavailability reports: partner update own" ON public.unavailability_reports;

CREATE POLICY "unavailability reports: partner create own"
ON public.unavailability_reports
FOR INSERT
TO authenticated
WITH CHECK (partner_id = auth.uid());

CREATE POLICY "unavailability reports: partner admin customer read"
ON public.unavailability_reports
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "unavailability reports: partner update own"
ON public.unavailability_reports
FOR UPDATE
TO authenticated
...
DROP POLICY IF EXISTS "dirty reports: partner admin read" ON public.dirty_vehicle_reports;
DROP POLICY IF EXISTS "dirty reports: partner admin customer read" ON public.dirty_vehicle_reports;
CREATE POLICY "dirty reports: partner admin customer read"
