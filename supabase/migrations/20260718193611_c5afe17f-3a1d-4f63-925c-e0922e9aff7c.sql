...
-- 1. Deduplicate offer_next_for_queue notification insert to one-per-(partner, booking).
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE o.partner_id = v_pick.partner_id AND o.response='pending'
  ...
  -- Deduplicate: one live notification per (partner, booking). Update in place
  -- across retries; only insert if we've never notified this partner about
  -- this booking before.
...
-- 2. Trigger: when a subscription_offer moves to a terminal state, archive
-- the matching partner_notification (mark read) so the inbox doesn't pile up.
...
BEGIN
  IF NEW.response IS DISTINCT FROM OLD.response
     AND NEW.response IN ('declined','expired','cancelled','accepted') THEN
    UPDATE public.partner_notifications
...
DROP TRIGGER IF EXISTS trg_archive_offer_notification ON public.subscription_offers;
CREATE TRIGGER trg_archive_offer_notification
AFTER UPDATE ON public.subscription_offers
FOR EACH ROW EXECUTE FUNCTION public.tg_archive_offer_notification();

-- 3. Backfill: archive every existing daily_shine_offer notification whose
-- referenced offer is no longer pending (or is gone).
UPDATE public.partner_notifications pn
   SET read_at = COALESCE(pn.read_at, now())
 WHERE pn.type = 'daily_shine_offer'
   AND pn.read_at IS NULL
   AND (
     NOT EXISTS (
       SELECT 1 FROM public.subscription_offers so
       WHERE so.id::text = pn.metadata->>'offer_id'
     )
     OR EXISTS (
       SELECT 1 FROM public.subscription_offers so
       WHERE so.id::text = pn.metadata->>'offer_id'
         AND (so.response <> 'pending' OR so.expires_at <= now())
     )
   );

-- 4. Collapse historical duplicates: for each (partner, booking) keep only
-- the newest daily_shine_offer notification; archive the rest.
WITH ranked AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY partner_id, (metadata->>'booking_id')
...
UPDATE public.partner_notifications pn
   SET read_at = COALESCE(pn.read_at, now())
  FROM ranked r
 WHERE pn.id = r.id AND r.rn > 1 AND pn.read_at IS NULL;
