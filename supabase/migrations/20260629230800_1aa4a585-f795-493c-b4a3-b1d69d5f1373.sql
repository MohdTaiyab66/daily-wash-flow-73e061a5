-- Avoid double admin alerts: trigger already inserts the refund alert on the
-- payment_status='paid' transition. RPC keeps the customer notification +
-- refund_required flag but no second alert.
...
DECLARE
  ...
  v_booking record;
  v_subscription uuid; v_payment uuid; v_queue uuid; v_lead uuid;
...
BEGIN
  SELECT bk.*, sc.slug AS service_slug, sc.service_type, sc.name AS service_name, sc.category AS service_category
    INTO v_booking FROM public.bookings bk
    JOIN public.service_catalog sc ON sc.id = bk.service_id WHERE bk.id = p_booking_id;
  ...
  IF v_booking.service_category = 'subscription' OR v_booking.service_type='subscription' THEN
    ...
    IF v_dup_sub_id IS NOT NULL THEN
      ...
      -- Trigger has already raised admin_alerts(kind='refund_required'). Notify customer only.
    ...
    ELSE
      -- Trigger has typically already inserted the subscription + queue.
      -- The block below is idempotent: ON CONFLICT(booking_id) does nothing harmful.
      ...
      -- Notify customer once. (Trigger doesn't send the customer notification.)
      INSERT INTO public.customer_notifications(user_id,type,title,body,link,metadata)
      SELECT v_booking.user_id,'subscription_paid','Daily Shine subscription active',
      ...
      WHERE NOT EXISTS (
        ...
        WHERE user_id = v_booking.user_id AND type='subscription_paid'
  ...
  ELSE
    ...
    INSERT INTO public.service_leads(
      ...
      v_booking.vehicle_id, v_veh_label, v_booking.address_id, v_addr,
    ...
    ON CONFLICT DO NOTHING RETURNING id INTO v_lead;
