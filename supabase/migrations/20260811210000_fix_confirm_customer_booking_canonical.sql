-- URGENT PAYMENT BLOCKER FIX: Align RPC with canonical DB states.
-- 1. Ensure duplicate check only blocks on 'paid' bookings.
-- 2. Use 'pending_payment' for status and 'pending' for payment_status.
...
BEGIN
  ...
  -- CRITICAL: Only block if there is a subscription linked to a SUCCESSFULLY PAID booking.
  -- Subscriptions created by confirm_customer_booking (if any) or stale unpaid ones should not block.
