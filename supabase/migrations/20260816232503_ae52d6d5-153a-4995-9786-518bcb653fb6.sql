...
AS $function$
 BEGIN
   -- Sweeps: ensure we don't return expired/superseded stuff
   -- (Optional logic here if we need to mark things as expired, but the query below filters them)
   ...
   SELECT
     mb.id as b_id,
     p_partner_id as prt_id,
     mb.current_incentive as inc,
     'pending' as resp,
     mb.current_round as rnd
   ...
   SELECT
     ...
     b.booking_id,
     now() as srv_now,
     (CASE WHEN b.assignment_id IS NOT NULL THEN 26 ELSE 1 END)::integer as cust_count,
     (b.current_incentive * 26) as earn_monthly,
     b.current_incentive as earn_amount,
     'Nearby'::text as dist_display,
     cz.name as area_name,
     b.assignment_id
