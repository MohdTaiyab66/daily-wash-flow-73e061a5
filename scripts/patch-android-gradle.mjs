...
// The Razorpay SDK pin lives PERMANENTLY in android/app/build.gradle
// ([uw-payments] block) — never re-inject it here. Strip any legacy
// script-injected pin so there is exactly one declaration.
const LEGACY_PIN_BEGIN = "// [uw-razorpay-pin BEGIN]";
const LEGACY_PIN_END = "// [uw-razorpay-pin END]";
while (gradle.includes(LEGACY_PIN_BEGIN) && gradle.includes(LEGACY_PIN_END)) {
  const start = gradle.indexOf(LEGACY_PIN_BEGIN);
  const end = gradle.indexOf(LEGACY_PIN_END, start);
  if (start === -1 || end === -1) break;
  gradle = gradle.slice(0, start) + gradle.slice(end + LEGACY_PIN_END.length).replace(/^\r?\n/, "");
