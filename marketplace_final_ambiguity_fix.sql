-- marketplace_final_ambiguity_fix.sql
-- Fixes "column reference broadcast_id is ambiguous" by qualifying columns in ON CONFLICT

-- 1. Update mp_reconcile_all_partners_for_broadcast
...
BEGIN
  ...
  SELECT 
    v_bcast.id as broadcast_id,
    p.id as partner_id,
    v_bcast.current_incentive as incentive,
    'pending' as response,
    v_bcast.current_round as round
...
-- 2. Update get_partner_open_offers
