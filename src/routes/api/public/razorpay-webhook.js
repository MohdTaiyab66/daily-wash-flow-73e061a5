    ...
    server: {
        handlers: {
            POST: async ({ request }) => {
                ...
                if (!booking)
                    return Response.json({ ok: true, ignored: true, reason: "booking_not_found" });
                const { data, error } = await supabaseAdmin.rpc("activate_paid_booking", {
                ...
                try {
                    await supabaseAdmin.rpc("ds_on_payment_verified", { p_booking_id: booking.id });
                }
                catch { }
                ...
                try {
                    ...
                    console.warn("[razorpay-webhook] immediate push dispatch failed", e);
                }
