...
import { Pause, ShieldAlert, Settings2, XCircle, ChevronRight } from "lucide-react";
...
import { useSelectedVehicleId } from "@/components/customer/VehicleSelector";
...
import { getActiveSubscriptionForVehicle } from "@/lib/subscription-cancel.functions";
...
function MyPlanPage() {
    ...
    const [selectedVehicleId, setSelectedVehicleId] = useSelectedVehicleId(vehiclesQ.data, preferredSubQ.data ?? null);
    ...
    const subs = all.filter((b) => b.service_catalog?.service_type === "subscription" && b.payment_status === "paid");
    const pendingSub = all.find((b) => b.service_catalog?.service_type === "subscription" &&
        b.payment_status !== "paid" &&
        b.status !== "cancelled" &&
        b.status !== "expired" &&
        b.status !== "refunded") ?? null;
    const activeSub = subs.find((s) => s.status !== "cancelled" && s.status !== "expired" && new Date(s.scheduled_date) <= new Date()) ?? subs[0];
    ...
    const interiorReal = all.filter((b) => (b.service_catalog?.slug?.includes("interior") || b.service_catalog?.slug?.includes("deep")) &&
        new Date(b.scheduled_date) >= monthStart &&
        b.status === "completed");
    const exteriorReal = all.filter((b) => (b.service_catalog?.slug?.includes("exterior") || b.service_catalog?.slug?.includes("basic") || b.service_catalog?.slug?.includes("daily")) &&
        new Date(b.scheduled_date) >= monthStart &&
        b.status === "completed");
    ...
    return (<div className="min-h-screen bg-[#FDFDFD] pb-24">
      <UWHeader area="My Plan" hideLocationIcon onAreaClick={() => { }} activeVehicle={selectedVehicle ? {
            make: selectedVehicle.make,
            model: selectedVehicle.model,
            registration_number: selectedVehicle.registration_number || "",
            category: 'sedan',
            color: null
        } : undefined} onVehicleClick={() => {
      ...
      <div className="px-5 pt-5">
        {hasVehicles && (<>
            {bookingsQ.isLoading ? (<div className="mt-6 space-y-4">
                <Shimmer className="h-48 w-full rounded-2xl"/>
                <Shimmer className="h-20 w-full rounded-2xl"/>
              </div>) : (<>
                {!activeSub && pendingSub && (<PendingPaymentCard booking={pendingSub} vehicleId={selectedVehicleId}/>)}

                {!activeSub && !pendingSub && (<NoSubscriptionState vehicleId={selectedVehicleId} vehicleLabel={vehicleLabel}/>)}

                {activeSub && (<div className="mt-4 space-y-4">
                    <AwaitingPartnerBanner userId={userId} vehicleId={selectedVehicleId}/>
                    
                    {latestNoticeQ.data && (<Surface className="border-primary/20 bg-white p-5">
                      ...
                      <div className="space-y-6">
                        <div>
                          ...
                          <div className="h-1.5 w-full bg-black/5 rounded-full overflow-hidden">
                            <div className="h-full bg-[#FF6B00] rounded-full transition-all duration-500" style={{ width: `${(elapsed / 25) * 100}%` }}/>
                          </div>
                        </div>
                        ...
                        <div className="pt-1">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <p className={cn("text-[12px] font-black", interiorCount > 0 ? "text-[#FF6B00]" : "text-black/30")}>
                              {interiorCount > 0 ? "1 / 1 USED" : "0 / 1 USED"}
                            </p>
                          </div>
                          <div className="h-1.5 w-full bg-black/5 rounded-full overflow-hidden">
                            <div className={cn("h-full rounded-full transition-all duration-500", interiorCount > 0 ? "bg-[#FF6B00]" : "bg-black/10")} style={{ width: interiorCount > 0 ? "100%" : "0%" }}/>
                          </div>
                        </div>
                      </div>
      ...
      <CancelPlanDialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen} subscriptionId={subRow?.id ?? null} renewalDate={subRow?.renewal_date ?? planEnd ?? null} planName={activeSub?.service_catalog?.name ?? "Daily Shine"}/>
      <BookAWashSheet open={bookOpen} onOpenChange={setBookOpen} vehicleId={selectedVehicleId} userId={userId}/>
      <PackageBuilderSheet open={builderOpen} onOpenChange={setBuilderOpen} basePlanSlug={activePlanSlug ?? "daily_shine_monthly"} basePlanPrice={(() => {
            const dailyShineService = services?.find((s) => s.slug === 'daily-shine');
            const resolvedPrice = resolveDailyShinePrice(selectedVehicle?.category || undefined, dailyShineService);
            const dbAmount = subRow?.amount ?? activeSub?.total_amount;
            if (selectedVehicle?.category === 'hatchback_compact_sedan')
                return 999;
            if (selectedVehicle?.category === 'sedan_suv')
                return 1199;
...
function PendingPaymentCard({ booking, vehicleId }) {
    ...
    return (<div className="mt-5 overflow-hidden rounded-[22px] border border-[#EEEEEE] bg-white p-5 shadow-sm">
