...
import { ArrowLeft, ChevronRight, Loader2, Sparkles, Car, X, ZapIcon as ZapIconLucide, Plus, Minus, ShoppingCart, Waves, Droplet, Wind, ShieldCheck, CalendarDays } from "lucide-react";
...
function ServiceDetail() {
    ...
    const vehiclesQ = useQuery({
        ...
        queryFn: async () => {
            ...
            if (error)
                console.error("[SERVICE] VEHICLES_ERROR", error);
            return (data ?? []);
        }
    });
    const [vehicleId, setVehicleId] = useState(search.vehicleId || null);
    ...
    const updateDiagStep = (id, status, error, details) => {
        setDiag(prev => ({
            ...prev,
            steps: prev.steps.map(s => s.id === id ? { ...s, status, time: new Date().toLocaleTimeString(), error, details } : s)
        }));
    };
    ...
    const addressesQ = useQuery({
        ...
        queryFn: async () => {
            ...
            if (error)
                console.error("[SERVICE] ADDRESSES_ERROR", error);
            return (data ?? []);
        }
    });
    ...
    const addonsQ = useQuery({
        ...
        queryFn: async () => {
            ...
            if (error)
                console.error("[SERVICE] ADDONS_ERROR", error);
            return (data ?? []);
        }
    });
    ...
    const galleryImages = useMemo(() => {
        ...
        return items.length > 0 ? items.map((i) => i.image_url) : (getServiceImage(slug).url ? [getServiceImage(slug).url] : []);
    }, [galleryQ.data, slug]);
    const confirm = async () => {
        setDiag(prev => ({
            ...prev,
        ...
        try {
            ...
            setDiag(prev => {
                ...
                if (firstPendingIndex !== -1) {
                    return {
                        ...prev,
                        steps: steps.map((s, idx) => idx === firstPendingIndex ? { ...s, status: 'err', error: e.message } : s)
                    };
                }
                return prev;
            });
    ...
    const getAddonQty = (id) => cartItems.find(i => i.id === id)?.quantity || 0;
    const QuantityControl = ({ id, name, price, type }) => {
        ...
        return (<div className="flex items-center gap-3 bg-[#F1F2F3] rounded-full px-2 py-1 border border-black/5">
        <button onClick={handleDecrease} disabled={type === 'base' && qty <= 1} className={cn("w-7 h-7 rounded-full flex items-center justify-center transition-all active:scale-90", type === 'base' && qty <= 1 ? "text-black/10" : "text-[#1a1a1a] hover:bg-black/5")}>
          <Minus className="h-3.5 w-3.5"/>
        </button>
        <span className="text-[13px] font-black min-w-[12px] text-center">{qty}</span>
        <button onClick={handleIncrease} className="w-7 h-7 rounded-full flex items-center justify-center text-[#EA580C] hover:bg-[#EA580C]/5 transition-all active:scale-90">
          <Plus className="h-3.5 w-3.5"/>
        </button>
      </div>);
    };
    if (serviceQ.isPending) {
        return (<div className="flex min-h-screen items-center justify-center bg-[#FAF9F7]">
    ...
    if (serviceQ.isError || !service) {
        return (<div className="flex min-h-screen flex-col items-center justify-center bg-[#FAF9F7] px-6 text-center">
        ...
        <Button onClick={() => navigate({ to: "/c/home" })} className="bg-[#1a1a1a] text-white rounded-full font-black px-8">
          Back to Home
        </Button>
      </div>);
    }
    if (vehiclesQ.isSuccess && !vehicle) {
        ...
        <Button onClick={() => navigate({ to: "/c/home" })} className="bg-[#FF6B00] text-white rounded-full font-black px-10 h-12 w-full max-w-xs shadow-lg shadow-orange-200">
          Select Vehicle
        </Button>
      </div>);
    }
    return (<div className="min-h-screen bg-[#FAF9F7] pb-[180px]">
      ...
      <div className="px-6 pt-6 w-full box-border">
        <div className="overflow-hidden rounded-[18px] border border-[#2D2D2D]/8 shadow-sm bg-white relative w-full aspect-[2/1]" ref={emblaRef}>
          <div className="flex h-full">
             {galleryImages.map((img, i) => (<div key={i} className="flex-[0_0_100%] w-full h-full">
                 <img src={img} className="w-full h-full object-cover" alt={`${service.name} ${i + 1}`}/>
               </div>))}
          </div>
        </div>
        {galleryImages.length > 1 && (<div className="flex justify-center gap-1.5 mt-3">
      ...
      <div className="px-6 mt-6 space-y-5 max-w-full">
        ...
        {/* Dynamic Includes */}
        {service.inclusions_json && (<div className="bg-white p-5 rounded-[16px] border border-black/[0.05] shadow-sm grid grid-cols-3 gap-y-5 gap-x-2 w-full box-border">
            {service.inclusions_json.map((item, i) => {
                const label = item.label.toLowerCase();
                const Icon = label.includes('pressure') ? Waves :
                                                        ...
                                                        label.includes('scheduled') ? CalendarDays :
                                                            label.includes('assurance') ? ShieldCheck :
                                                                label.includes('exterior') ? Droplet :
                                                                    ZapIconLucide;
                return (<div key={i} className="flex flex-col items-center text-center gap-2 min-w-0">
        ...
        <div className="bg-white p-5 rounded-[16px] border border-black/[0.05] shadow-sm w-full box-border">
          ...
          <div className="grid grid-cols-3 gap-2">
            {TIME_SLOTS.map(t => (<button key={t} onClick={() => setSlot(t)} className={cn("py-2 rounded-lg border font-bold text-[10px] transition-all", slot === t ? "bg-[#FFF2ED] border-[#EA580C] text-[#EA580C]" : "bg-white border-black/[0.05] text-[#1a1a1a]")}>{t}</button>))}
          </div>
        </div>

        {/* Premium Add-ons */}
        {relevantAddons.length > 0 && (<div className="bg-white p-5 rounded-[16px] border border-black/[0.05] shadow-sm w-full box-border">
            <div className="flex justify-between items-center mb-4">
            ...
            <div className="space-y-4">
              {relevantAddons.slice(0, 3).map(a => {
                ...
                return (<div key={a.id} className="flex items-center justify-between gap-3 p-1">
        ...
        <div className="bg-white p-5 rounded-[16px] border border-black/[0.05] shadow-sm w-full box-border mb-8">
          ...
          <div className="space-y-2">
            {cartItems.map(item => (<div key={item.id} className="flex justify-between text-[13px]">
      ...
      <Drawer open={showAddonDrawer} onOpenChange={setShowAddonDrawer}>
        <DrawerContent className="h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6">
            ...
            const addonPrice = isSUV ? a.price_sedan_suv : a.price_hatchback;
            return (<div key={a.id} className="flex items-center justify-between p-4 bg-[#F1F2F3]/50 rounded-xl border border-black/5">
      ...
      <Drawer open={showCartDrawer} onOpenChange={setShowCartDrawer}>
        <DrawerContent className="h-[70vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6">
            <div className="space-y-4 pb-[160px]">
              {cartItems.map(item => (<div key={item.id} className="flex items-center justify-between p-4 bg-[#F1F2F3]/50 rounded-xl">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[80] w-full flex flex-col pointer-events-none pb-[env(safe-area-inset-bottom,16px)]">
        {/* Compact Cart Bar */}
        {!showAddonDrawer && !showCartDrawer && totalItems > 0 && (<div className="px-6 mb-3 pointer-events-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
            <button onClick={() => setShowCartDrawer(true)} className="w-full bg-[#1a1a1a] text-white h-[56px] rounded-[24px] px-5 flex items-center justify-between shadow-[0_12px_35px_rgba(0,0,0,0.35)] active:scale-[0.98] transition-all border border-white/5">
        ...
        <div className="bg-white border-t border-black/[0.05] p-6 flex justify-between items-center shadow-[0_-12px_40px_rgba(0,0,0,0.08)] pointer-events-auto gap-4 min-h-[88px]">
          ...
          <Button onClick={(e) => {
            ...
            confirm();
        }} disabled={submitting || !slot} className={cn("h-[54px] w-[180px] rounded-[18px] bg-[#EA580C] text-white font-[900] text-[15px] active:scale-[0.96] transition-all shadow-[0_8px_25px_rgba(234,88,12,0.25)] relative z-[99]", (submitting || !slot) && "opacity-50 grayscale shadow-none")}>
            {submitting ? <Loader2 className="h-5 w-5 animate-spin"/> : (<span className="flex items-center gap-2 pointer-events-none">
      ...
      <Drawer open={diag.open} onOpenChange={(o) => setDiag(prev => ({ ...prev, open: o }))} dismissible={true}>
        <DrawerContent className="max-h-[90vh] bg-[#0A0A0A] text-white font-mono p-6 border-t border-white/10">
          ...
          <div className="space-y-3 overflow-y-auto pb-12 pr-1">
            {diag.orderInfo && (<div className="bg-white/5 border border-white/10 p-3 rounded-xl mb-4">
            ...
            {diag.steps.map((step, idx) => (<div key={step.id} className={cn("flex flex-col gap-1 p-2 rounded-lg transition-colors", step.status === 'pending' ? "opacity-30" : "bg-white/5")}>
                <div className="flex items-center justify-between">
                  <span className={cn("text-[12px] font-bold", step.status === 'ok' ? "text-white" :
                step.status === 'err' ? "text-[#f87171]" : "text-white/60")}>
                    {step.label}
                  </span>
                  <div className="flex items-center gap-2">
                    {step.time && <span className="text-[9px] text-white/30">{step.time}</span>}
                    <span className={cn("text-[11px] font-black w-5 h-5 flex items-center justify-center rounded-full", step.status === 'ok' ? "bg-[#4ade80]/20 text-[#4ade80]" :
                step.status === 'err' ? "bg-[#f87171]/20 text-[#f87171]" : "bg-white/10 text-white/20")}>
                      {step.status === 'ok' ? '✓' : step.status === 'err' ? '✗' : '...'}
                    </span>
                  </div>
                </div>
                {step.error && (<div className="text-[10px] text-[#f87171]/80 leading-tight pl-0 mt-1 font-medium bg-[#f87171]/5 p-2 rounded border border-[#f87171]/10">
                    ERR: {step.error}
                  </div>)}
                {step.details && (<div className="text-[10px] text-[#4ade80]/80 font-black tracking-widest mt-0.5">
                    {step.details}
                  </div>)}
              </div>))}
          </div>
