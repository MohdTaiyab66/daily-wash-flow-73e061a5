  ...
  return (
    <PullToRefresh onRefresh={refreshAll}>
      {/* '''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''
                                            ...
                                            🚨 P0 — REAL-TIME PARTNER BOOKING PUSH + VEHICLE UNAVAILABLE PUSH

IMPORTANT: CUSTOMER FCM / ANDROID DELIVERY IS NOW PHYSICALLY VERIFIED.

We now have TWO separate production notification failures that must be traced and fixed.

==================================================

PROBLEM 1 — NEW CUSTOMER BOOKING → PARTNER

==================================================

Partner does NOT receive the notification immediately.

Then:

Partner opens the Partner App → notification suddenly appears.

This strongly indicates that the notification is being created/queued but the real-time dispatch trigger is missing.

REQUIRED LOGGING (Partner Side):

[PARTNER-BOOKING-E2E:01]
CUSTOMER_BOOKING_CREATED

[PARTNER-BOOKING-E2E:02]
ELIGIBLE_PARTNERS_RESOLVED

[PARTNER-BOOKING-E2E:03]
PARTNER_NOTIFICATION_CREATED

[PARTNER-BOOKING-E2E:04]
REALTIME_DISPATCH_TRIGGERED

[PARTNER-BOOKING-E2E:05]
PARTNER_TOKEN_RESOLVED

[PARTNER-BOOKING-E2E:06]
FCM_SEND_STARTED

[PARTNER-BOOKING-E2E:07]
FCM_SERVER_ACCEPTED

==================================================

PROBLEM 2 — VEHICLE UNAVAILABLE → CUSTOMER

==================================================

Partner marks the vehicle unavailable.

Expected:

Customer immediately receives notification.
...
Inspect the actual database row/event generated by the unavailable flow.

Determine the exact stored notification type.

REQUIRED LOGGING (Customer Side):

[UNAVAILABLE-E2E:01]
PARTNER_UNAVAILABLE_ACTION
...
1. Booking Real-time Trigger:
Ensure every checkout confirmation (verifyRazorpayPayment and razorpay-webhook) immediately triggers the partner notification dispatcher.

2. Unavailable Notification Mapping:
Verify the notification type used in GuidedReport.tsx and submit_service_unavailable matches the allow-list in dispatch.server.ts and the native Android receiver.

3. Forensic Trace:
The logs must prove exactly where the chain breaks.

==================================================

BUILD: FCM-P0-REALTIME-FIX-08
================================================== */ }
