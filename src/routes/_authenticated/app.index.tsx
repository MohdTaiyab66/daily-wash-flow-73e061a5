...
function HomePage() {
  ...
  if (assignment) {
    ...
    dayLabel = `Day ${Math.max(1, Math.min(dayNum, assignment.duration_days))} of ${assignment.duration_days}`;
  }
  ...
  return (
    <div className="mx-auto max-w-md px-5 pt-5">
      ...
      {assignment ? (
        <Card className="mt-5 border-0 bg-foreground p-5 text-background">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-background/60">Current assignment</p>
              <p className="mt-1 text-lg font-semibold">{assignment.area}</p>
              <p className="mt-0.5 text-xs text-background/60">{dayLabel} · starts {assignment.expected_start_time}</p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 border-t border-background/10 pt-4 text-xs">
            ...
            <div><p className="text-background/60">Distance</p><p className="mt-0.5 text-base font-semibold">{remDistance} km</p></div>
            <div><p className="text-background/60">ETA</p><p className="mt-0.5 text-base font-semibold">{remTime}h</p></div>
            <div><p className="text-background/60">Earned</p><p className="mt-0.5 text-base font-semibold">₹{earnings}</p></div>
          </div>
        ...
        <Card className="mt-5 flex flex-col items-center gap-3 p-8 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Build your own assignment — choose cars and duration.</p>
          </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        <Stat icon={<Car className="h-4 w-4" />} label="Today" value={`₹${earnings}`} />
      ...
      <div className="mt-3 grid grid-cols-3 gap-3">
        ...
        <Stat icon={<Award className="h-4 w-4" />} label="Level" value={partner?.level ?? "Bronze"} />
      </div>
      ...
      <div className="h-4" />
      <p className="mb-2 text-center text-[10px] text-muted-foreground"><Clock className="mr-1 inline h-3 w-3" />Customer locations show only when you are online.</p>
    </div>
  );
}
