...
function RoutePage() {
    useRealtimeInvalidation(["services", "assignments", "customers", "vehicles", "dirty_vehicle_reports", "unavailability_reports", "wallet_ledger", "customer_notifications", "admin_alerts"], [["route-today"], ["today-assignment"], ["today-assignment"], ["earnings-v3"], ["wallet-balance"]]);
    ...
    const { data: services } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!u.user)
                return loadRouteSnapshot("today", d) ?? [];
            ...
            if (error) {
                ...
                const cached = loadRouteSnapshot("today", d);
                if (cached)
                    return cached;
                throw error;
            }
    ...
    useEffect(() => {
        if (!services || !todayQuery.data)
            return;
        const liveTotal = (services ?? []).filter((s) => s.status !== "covered_by_booking").length;
        const sharedTotal = todayQuery.data.todaysCustomers;
        if (liveTotal !== sharedTotal) {
            console.warn("[assignment-consistency] Live vs shared today-assignment mismatch", { liveTotal, sharedTotal });
        }
    ...
    const unlockHH = Math.floor(unlockMsRemaining / 3600000);
    const unlockMM = Math.floor((unlockMsRemaining % 3600000) / 60000);
    const unlockSS = Math.floor((unlockMsRemaining % 60000) / 1000);
    ...
    const fmtHM = (d) => d ? d.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: false }) : "";
    ...
    const overrideMode = visibilityInfo?.override ?? "auto";
    ...
    const dirty = (services ?? []).filter((s) => s.status === "unavailable" && s.unavailable_reason === "dirty_vehicle");
    const unavailable = (services ?? []).filter((s) => s.status === "unavailable" && s.unavailable_reason !== "dirty_vehicle");
    const routeSource = (s) => {
        const snap = validateExactGps(s.destination_lat, s.destination_lng);
        if (snap)
            return { lat: snap.latitude, lng: snap.longitude, exact: true };
    ...
    const estFinishClock = mapStats?.mins
        ? new Date(nowTs + mapStats.mins * 60000).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", hour12: true }).toUpperCase()
        : null;
    ...
    const previewDate = total === 0 && activeAssignment && todayQuery.data?.nextDate
    ...
    const previewList = (previewServicesRaw ?? [])
        .filter((s) => s.status !== "covered_by_booking")
        .map((s, i) => {
        const snap = validateExactGps(s.destination_lat, s.destination_lng);
    ...
    const previewStartTime = previewList
        .map((s) => s.customers?.service_required_before ?? s.customers?.preferred_time ?? s.time_slot)
        .filter(Boolean)
        .sort()[0];
    const previewEarnings = previewList.reduce((sum, s) => sum + Number(s.rate_per_car ?? ratePerCar), 0);
    const [previewMapStats, setPreviewMapStats] = useState(null);
    ...
    return (<div className="mx-auto max-w-md px-5 pt-5 pb-10">
      <h1 className="text-2xl font-semibold tracking-tight">Route</h1>
      <div className="mt-1 flex items-center gap-2">
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${isPreviewMode ? "bg-primary/10 text-primary"
      ...
      <TodayAssignmentStatus isError={todayQuery.isError} isFetching={todayQuery.isFetching} isRefetching={todayQuery.isRefetching} hasData={todayQuery.data !== undefined} onRetry={() => todayQuery.refetch()} metrics={todayQuery.metrics} lastSuccessAt={todayQuery.data?.fetchedAt ?? todayQuery.lastGood?.fetchedAt ?? null}/>

      <div className="mt-4"><DarOfferCard /></div>

      {/* Offline banner — reads survive short outages via the last-known snapshot. */}
      {!netOnline && (<div className="mt-4 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5">
      ...
      {isPreviewMode && (<div className="mt-4 flex items-start gap-2 rounded-lg border border-primary/25 bg-primary/8 px-3 py-2.5">
      ...
      {isPreviewMode && previewList.length > 0 && (<Card className="mt-4 p-4">
      ...
      <div className="mt-5">
        <LiveMap stops={activeStops} showCustomers={activeStops.length > 0} heightClass="h-40" hideStats onStats={isPreviewMode ? setPreviewMapStats : setMapStats}/>
      </div>

      {/* NEXT CUSTOMER — hero (working days and preview both) */}
      {!isEndOfDay && routeUnlocked && activeNext && (<NextCustomerHero stop={activeNext} seqNo={activeSeqStart} total={activeTotal} locked={isPreviewMode} previewDayLabel={isPreviewMode ? previewDayLabel : undefined}/>)}

      {/* Locked / empty states */}
      {!routeUnlocked && (<Card className="mt-5 p-6 text-center">
          ...
          {overrideMode !== "hide" && visibilityUnlockAt && unlockMsRemaining > 0 && (<p className="mt-3 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-semibold tabular-nums text-primary">
      ...
      {routeUnlocked && !isPreviewMode && !isEndOfDay && pending.length === 0 && (<Card className="mt-5 p-6 text-center text-sm text-muted-foreground">No pending stops.</Card>)}

      {/* Today's Progress — same card on working & preview days */}
      {activeTotal > 0 && !isEndOfDay && (<Card className="mt-5 p-4">
      ...
      {!isEndOfDay && routeUnlocked && activeQueue.length > 0 && (<section className="mt-6">
          ...
          <div className="space-y-2">
            {activeQueue.map((s, idx) => (<QueueRow key={s.id} stop={s} seqNo={activeSeqStart + idx + 1} total={activeTotal} locked={isPreviewMode}/>))}
          </div>
        </section>)}

      {/* End of day success */}
      {isEndOfDay && (<Card className="mt-5 border-[color:var(--success)]/30 bg-[color:var(--success)]/5 p-5 text-center">
      ...
      {/* Completed — collapsed */}
      {completed.length > 0 && (<CollapsibleSection title="Completed" count={completed.length} accent="success" defaultOpen={false}>
          <div className="space-y-1">
            {completed.map((s) => {
                const c = s.customers;
                return (<div key={s.id} className="flex items-center justify-between gap-3 rounded-md px-2 py-1.5">
      ...
      {(dirty.length + unavailable.length) > 0 && (<CollapsibleSection title="Service exceptions" count={dirty.length + unavailable.length} accent="warning" defaultOpen={false} subtitle={<span className="flex items-center gap-3 text-[11px] text-muted-foreground">
              {dirty.length > 0 && (<span className="inline-flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-primary"/> Dirty ({dirty.length})
                </span>)}
              {unavailable.length > 0 && (<span className="inline-flex items-center gap-1">
...
function NextCustomerHero({ stop, seqNo, total, locked, previewDayLabel, unlockCountdown }) {
    const c = stop.customers;
    const v = stop.vehicles;
    const gps = { lat: stop.lat, lng: stop.lng };
    ...
    return (<Card className="mt-5 overflow-hidden border border-border bg-background p-0 shadow-[0_18px_44px_-18px_hsl(var(--primary)/0.35)]">
      ...
      <div className="flex items-start gap-3.5 p-4">
        <div className="relative shrink-0">
          <TappableVehicleImage path={v?.front_image_path} className="h-[104px] w-[104px] rounded-xl" alt={`${v?.make ?? ""} ${v?.model ?? ""}`} customerName={c?.full_name} vehicleLabel={`${v?.make ?? ""} ${v?.model ?? ""}`.trim()} registration={v?.registration_number}/>
          {v?.front_image_path && (<span className="pointer-events-none absolute bottom-1 right-1 grid h-5 w-5 place-items-center rounded-full bg-black/70 text-white shadow">
        ...
        <div className="min-w-0 flex-1">
          ...
          {v?.registration_number && (<span className="mt-1 inline-block rounded-md border border-foreground/70 bg-yellow-50 px-1.5 py-0.5 font-mono text-[10px] font-bold tracking-wider text-foreground">
              {v.registration_number}
            </span>)}
          {locked ? (<div className="mt-1.5 space-y-1">
              {scheduledTime && (<p className="text-[11px] text-muted-foreground">
            ...
            </div>) : ((c?.service_required_before || c?.preferred_time) && (<p className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-primary/15 px-2 py-0.5 text-[11px] font-medium text-primary">
      ...
      <div className="grid grid-cols-[1fr_1fr_2.2fr] gap-2 border-t border-border/60 bg-muted/20 p-3">
        <IconAction icon={<Navigation className="h-5 w-5"/>} ariaLabel="Open maps" disabled={!navUrl || locked} onClick={async () => {
            if (locked)
                return;
            await openGoogleMapsDirections(gps.lat, gps.lng);
        }}/>
        {locked ? (<IconAction icon={<Phone className="h-5 w-5"/>} ariaLabel="Call customer" disabled onClick={() => { }}/>) : (<HeroCallIconAction serviceId={stop.id}/>)}
        {locked ? (<button type="button" disabled className="flex items-center justify-center gap-2 rounded-lg bg-muted px-3 py-2.5 text-sm font-bold uppercase tracking-wide text-muted-foreground shadow-sm">
            <Lock className="h-4 w-4"/>
            <span>{unlockCountdown ? `Unlocks in ${unlockCountdown}` : `Available ${previewDayLabel ?? "Tomorrow"}`}</span>
          </button>) : (<Link to="/app/service/$id" params={{ id: stop.id }} className="flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2.5 text-sm font-bold uppercase tracking-wide text-primary-foreground shadow-md transition-colors hover:bg-primary/90">
...
function IconAction({ icon, ariaLabel, onClick, disabled }) {
    return (<button type="button" onClick={onClick} disabled={disabled} aria-label={ariaLabel} className="grid h-11 place-items-center rounded-lg border border-border/60 bg-background text-foreground shadow-sm transition-colors hover:bg-muted/40 disabled:opacity-50">
      {icon}
    </button>);
}
function HeroCallIconAction({ serviceId }) {
    ...
    return (<IconAction icon={loading ? <Loader2 className="h-5 w-5 animate-spin"/> : <Phone className="h-5 w-5"/>} ariaLabel="Call customer" onClick={onClick}/>);
}
function QueueRow({ stop, seqNo, total: _total, locked, unlockCountdown }) {
    const c = stop.customers;
    const v = stop.vehicles;
    const gps = { lat: stop.lat, lng: stop.lng };
    const navUrl = googleMapsDirectionsUrl(gps.lat, gps.lng);
    ...
    const rate = Number(stop.rate_per_car ?? 17);
    const scheduledTime = c?.service_required_before ?? c?.preferred_time ?? stop.time_slot;
    const inner = (<Card className={`flex items-center gap-2.5 p-2.5 ${locked ? "opacity-95" : ""}`}>
      <div className="w-7 shrink-0 text-center text-xs font-bold tabular-nums text-muted-foreground">
        #{seqNo}
      </div>
      <TappableVehicleImage path={v?.front_image_path} className="h-12 w-12 shrink-0 rounded-lg" alt={`${v?.make ?? ""} ${v?.model ?? ""}`} customerName={c?.full_name} vehicleLabel={`${v?.make ?? ""} ${v?.model ?? ""}`.trim()} registration={v?.registration_number}/>
      <div className="min-w-0 flex-1">
        ...
        {locked ? (<div className="mt-0.5 flex items-center gap-1.5">
            {scheduledTime && (<span className="text-[10px] font-medium text-foreground/80">
          ...
          </div>) : ((c?.service_required_before || c?.preferred_time) && (<p className="truncate text-[10px] text-primary">
              Before {formatTime12(c?.service_required_before ?? c?.preferred_time)}
            </p>))}
      </div>
      {!locked && (<div className="flex shrink-0 items-center gap-1.5">
          <button type="button" disabled={!navUrl} onClick={(e) => { e.preventDefault(); e.stopPropagation(); openGoogleMapsDirections(gps.lat, gps.lng); }} className="grid h-9 w-9 place-items-center rounded-full border border-border/60 bg-background text-foreground/80 shadow-sm transition-colors hover:bg-muted/40 disabled:opacity-40" aria-label="Open maps">
            <Navigation className="h-4 w-4"/>
          </button>
          <Link to="/app/service/$id" params={{ id: stop.id }} className="flex h-9 items-center gap-1.5 rounded-full bg-primary px-3.5 text-xs font-bold uppercase tracking-wide text-primary-foreground shadow-sm transition-colors hover:bg-primary/90" aria-label={inProgress ? "Resume service" : "Start service"}>
    ...
    if (locked) {
        return (<Link to="/app/service/$id" params={{ id: stop.id }} className="block">
        {inner}
      </Link>);
    }
    return inner;
}
function CollapsibleSection({ title, count, children, defaultOpen, accent, subtitle, }) {
    const dot = accent === "success"
        ? "bg-[color:var(--success)]"
        : accent === "warning"
            ? "bg-primary"
            : "bg-muted-foreground";
    return (<details className="group mt-4 rounded-lg border border-border/60 bg-background" open={defaultOpen}>
...
export function MaskedCallButton({ serviceId, compact, full, size }) {
    const call = useServerFn(initiateMaskedCall);
    const [loading, setLoading] = useState(false);
    const onClick = async () => {
        setLoading(true);
    ...
    if (compact) {
        return (<Button size="sm" variant="outline" onClick={onClick} disabled={loading} aria-label="Call Customer">
        {loading ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin"/> : <Phone className="mr-1.5 h-4 w-4"/>}Call
      </Button>);
    }
    return (<Button variant="outline" size={size ?? (full ? "lg" : "sm")} className={full ? "w-full" : ""} onClick={onClick} disabled={loading}>
