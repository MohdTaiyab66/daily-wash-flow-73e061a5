...
import { ArrowLeft, Camera, Check, ChevronDown, Loader2, Navigation, Clock, MapPin, ZoomIn, Play, MoreHorizontal, LifeBuoy, Phone, ShieldAlert, Sparkles, AlertTriangle, XCircle, } from "lucide-react";
...
import { PhotoSlot, getPosition } from "@/components/partner/service/photo-slot";
import { GuidedReport } from "@/components/partner/service/GuidedReport";
const AFTER_ANGLES = ["front", "rear", "left", "right"];
...
const stepKey = (id) => `uw_svc_step:${id}`;
const notesKey = (id) => `uw_svc_notes:${id}`;
function readStep(id) {
    if (typeof window === "undefined")
        return null;
    try {
        return window.localStorage.getItem(stepKey(id)) || null;
    }
    catch {
        return null;
    }
}
function writeStep(id, step) {
    try {
        window.localStorage.setItem(stepKey(id), step);
    }
    catch { /* noop */ }
}
...
function ServiceDetail() {
    ...
    const [serviceNotes, setServiceNotes] = useState(() => {
        if (typeof window === "undefined")
            return "";
        try {
            return window.localStorage.getItem(notesKey(id)) ?? "";
        }
        catch {
            return "";
        }
    });
    const [step, setStepState] = useState(() => readStep(id) ?? "arrival");
    const [celebration, setCelebration] = useState(null);
    const setStep = (next) => { writeStep(id, next); setStepState(next); };
    useEffect(() => { try {
        window.localStorage.setItem(notesKey(id), serviceNotes);
    }
    catch { /* noop */ } }, [id, serviceNotes]);
    ...
    const { data: nextServiceId } = useQuery({
        ...
        queryFn: async () => {
            ...
            const currentGps = validateExactGps(service?.destination_lat, service?.destination_lng);
    ...
    const { data: routeProgress } = useQuery({
        ...
        queryFn: async () => {
            ...
            const completed = rows.filter((r) => r.status === "completed" || r.status === "unavailable").length;
            const idx = rows.findIndex((r) => r.id === id);
            const position = idx >= 0 ? idx + 1 : Math.min(completed + 1, total || 1);
            const nextRow = idx >= 0 ? rows.slice(idx + 1).find((r) => r.status === "pending" || r.status === "in_progress") : null;
            return {
                total: total || 1,
                position,
                completed,
                nextName: nextRow?.customers?.full_name ?? null,
                nextArea: nextRow?.customers?.area ?? null,
            };
        },
    });
    const photoRows = photos ?? [];
    const beforeDone = photoRows.some((p) => p.stage === "before");
    const afterDone = new Set(photoRows.filter((p) => p.stage === "after").map((p) => p.angle));
    ...
    const complete = useMutation({
        mutationFn: async () => {
            if (!beforeDone)
                throw new Error("Take the before photo first");
            if (!afterAllDone)
                throw new Error("Take all 4 after photos first");
            const pos = await getPosition();
            const completedAt = new Date().toISOString();
            const { data, error } = await supabase.rpc("partner_complete_service", {
                p_service_id: id,
                p_lat: pos?.lat ?? null,
                p_lng: pos?.lng ?? null,
            ...
            if (error) {
                ...
                if (code === "P04PHOTO")
                    throw new Error("Some photos are missing. Please take them again.");
                throw new Error(error.message ?? "Could not complete service");
            }
            if (service?.started_at) {
                ...
                await supabase.from("service_analytics").upsert({
                    ...
                    area: service.customers?.area ?? null,
                    total_seconds: total,
        ...
        onSuccess: (data) => {
            try {
                window.localStorage.removeItem(stepKey(id));
    ...
    const goNext = async () => {
        ...
        if (fallbackNext)
            navigate({ to: "/app/service/$id", params: { id: fallbackNext } });
        else
    ...
    const c = service?.customers;
    const v = service?.vehicles;
    const exactDestination = validateExactGps(service?.destination_lat, service?.destination_lng);
    ...
    return (<div className="mx-auto max-w-md px-5 pt-4 pb-40">
      {celebration && (<ServiceCelebration open amount={celebration.amount} completed={celebration.completed} total={celebration.total} walletBalance={celebration.walletBalance ?? null} nextCustomerName={celebration.nextName ?? null} onDone={() => { setCelebration(null); void goNext(); }}/>)}
      ...
      <div className="flex items-center justify-between">
        ...
        {c?.area && (<span className="inline-flex items-center gap-1 rounded-full border border-border bg-muted/40 px-2.5 py-1 text-[11px] font-medium text-muted-foreground">
      ...
      {/* ---------------- DONE ---------------- */}
      {done && (<div className="mt-6 space-y-4 text-center">
          ...
          {routeProgress?.nextName && (<div className="rounded-2xl border border-border bg-card p-4 text-left">
      ...
      {/* ---------------- STEP 1 · ARRIVAL ---------------- */}
      {!done && step === "arrival" && (<>
          <CustomerCard service={service} c={c} v={v} timeLabel={timeLabel} hasNavigation={hasNavigation} destLat={destLat} destLng={destLng} serviceId={id} onOpenPhoto={() => setPhotoOpen(true)}/>
      ...
      {/* ---------------- STEP 2 · FIND VEHICLE ---------------- */}
      {!done && step === "found" && (<Question title="Did you find the vehicle?" onBack={() => setStep("arrival")} options={[
                { label: "Yes, I found it", tone: "primary", onClick: () => setStep("condition") },
                { label: "No, I did not", tone: "danger", onClick: () => setStep("notfound") },
            ]}/>)}

      {/* ---------------- NOT FOUND ---------------- */}
      {!done && step === "notfound" && (<div className="mt-6">
          <GuidedReport serviceId={id} assignmentId={service?.assignment_id ?? null} kind="unavailable" title="Why could you not do it?" reasons={NOT_FOUND_REASONS} angles={["front", "rear"]} photos={photoRows} refetch={() => void refetchPhotos()} compensation={COMPENSATION} onSubmitted={afterReport} onBack={() => setStep("found")}/>
        </div>)}

      {/* ---------------- CONDITION ---------------- */}
      {!done && step === "condition" && (<Question title="How is the vehicle?" onBack={() => setStep("found")} options={[
      ...
      {!done && (step === "dirty" || step === "attention") && (<div className="mt-6">
          <GuidedReport serviceId={id} assignmentId={service?.assignment_id ?? null} kind="dirty" title={step === "dirty" ? "What makes it dirty?" : "What is the problem?"} reasons={step === "dirty" ? DIRTY_REASONS : ATTENTION_REASONS} angles={["front", "rear", "left", "right"]} photos={photoRows} refetch={() => void refetchPhotos()} compensation={COMPENSATION} onSubmitted={afterReport} onBack={() => setStep("condition")} onContinueAnyway={() => setStep("before")} continueLabel="I can still clean it"/>
        </div>)}

      {/* ---------------- STEP 3 · BEFORE PHOTO ---------------- */}
      {!done && step === "before" && (<div className="mt-6 space-y-5">
          ...
          <PhotoSlot serviceId={id} assignmentId={service?.assignment_id ?? null} stage="before" angle="front" slotId="before" done={beforeDone} label="Before" variant="hero" hint="Whole car, before you start" onUploaded={() => { void refetchPhotos(); setStep("ready"); }}/>
        </div>)}
      ...
      {!done && step === "cleaning" && (<>
          ...
          {withinGrace && (<div className="mt-8 text-center">
          ...
          {!withinGrace && (<div className="mt-8">
              {!askCancelReason ? (<button type="button" onClick={() => setAskCancelReason(true)} className="mx-auto block text-sm font-medium text-muted-foreground underline">
                  Cancel service start
                </button>) : (<div className="space-y-3 rounded-2xl border border-border bg-card p-4">
      ...
      {/* ---------------- STEP 6 · AFTER PHOTOS ---------------- */}
      {!done && step === "after" && (() => {
            const labels = { front: "front", rear: "back", left: "left side", right: "right side" };
            const nextAngle = AFTER_ANGLES.find((a) => !afterDone.has(a));
            if (!nextAngle) {
                return (<div className="mt-8 space-y-4 text-center">
            ...
            <PhotoSlot key={nextAngle} serviceId={id} assignmentId={service?.assignment_id ?? null} stage="after" angle={nextAngle} slotId={nextAngle} done={false} label={labels[nextAngle]} variant="hero" hint="Keep the full car in the frame" onUploaded={() => void refetchPhotos()}/>
            <div className="flex items-center gap-2">
              {AFTER_ANGLES.map((a) => (<span key={a} className={`h-2 flex-1 rounded-full ${afterDone.has(a) ? "bg-[color:var(--success)]" : "bg-muted"}`}/>))}
            </div>
          </div>);
        })()}
      ...
      {!done && step === "summary" && (<>
          <div className="mt-6 space-y-4">
            ...
            <Textarea placeholder="Note for the customer or office (optional)" value={serviceNotes} onChange={(e) => setServiceNotes(e.target.value)} className="min-h-[90px] rounded-2xl text-base"/>
            <button type="button" onClick={() => setStep("after")} className="text-sm font-medium text-muted-foreground underline">
              Take a photo again
            </button>
          </div>
          <StickyBar>
            <Button size="lg" className="h-16 w-full text-lg font-bold" disabled={complete.isPending || !beforeDone || !afterAllDone} onClick={() => complete.mutate()}>
      ...
      <VehiclePhotoViewer open={photoOpen} onClose={() => setPhotoOpen(false)} photos={v?.front_image_path ? [{ path: v.front_image_path }] : []} customerName={c?.full_name} vehicleLabel={`${v?.make ?? ""} ${v?.model ?? ""}`.trim() || null} registration={v?.registration_number}/>
    </div>);
}
...
function StickyBar({ children }) {
    return (<div className="fixed inset-x-0 bottom-16 z-40 pointer-events-none px-5">
      <div className="pointer-events-auto mx-auto max-w-md">{children}</div>
    </div>);
}
function Question({ title, options, onBack, }) {
    return (<div className="mt-6 space-y-5">
      {onBack && (<button type="button" onClick={onBack} className="inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
          <ArrowLeft className="h-4 w-4"/> Back
        </button>)}
      <h2 className="text-3xl font-bold leading-tight">{title}</h2>
      <div className="space-y-3">
        {options.map((o) => (<button key={o.label} type="button" onClick={o.onClick} className={`flex min-h-[72px] w-full items-center gap-4 rounded-2xl border-2 px-5 text-left text-lg font-bold transition active:scale-[0.99] ${o.tone === "danger"
...
function MoreMenu({ open, onToggle, onNeedHelp, onReportIssue, }) {
    return (<div className="mt-4 overflow-hidden rounded-2xl border border-border bg-card">
...
function CustomerCard({ service, c, v, timeLabel, hasNavigation, destLat, destLng, serviceId, onOpenPhoto, }) {
    return (<div className="mt-4 overflow-hidden rounded-3xl border border-border bg-card">
      ...
      <div className="p-5">
        ...
        {v?.registration_number && (<span className="mt-2 inline-block rounded-md border border-foreground/30 bg-yellow-50 px-2 py-0.5 font-mono text-sm font-bold tracking-wider text-foreground">
        ...
        <ul className="mt-4 space-y-2 border-t border-border pt-3 text-base">
          ...
          {c?.address_line && (<li className="flex items-start gap-3">
        ...
        {(c?.special_instructions || c?.notes) && (<div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-3">
            <p className="whitespace-pre-line text-sm">{c.special_instructions || c.notes}</p>
          </div>)}
        <div className="mt-4 grid grid-cols-2 gap-2">
          <Button variant="outline" className="h-14 text-base" disabled={!hasNavigation} onClick={async () => {
