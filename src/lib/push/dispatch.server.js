...
async function listPendingOffers(sb) {
 ...
 */
export async function dispatchPendingOffers(claimedBy = "offer-push-dispatch") {
    ...
    for (const r of rows) {
        ...
        if (r.vehicle_category)
            data.vehicle = r.vehicle_category;
        try {
 ...
 */
export const CUSTOMER_ALLOWED_TYPES = new Set([
    // Payment lifecycle
    "payment_success",
    "payment_failed",
    "payment_cancelled",
 ...
 */
export const PARTNER_ASSIGNMENT_TYPES = new Set([
    "new_assignment",
    "new_assignments",
    "assignment_created",
    "assignment_updated",
 ...
 * android-native/kotlin/UrbanwashMessagingService.kt.
 */
export const CUSTOMER_HEADSUP_TYPES = new Set([
    "partner_accepted",
    "service_started",
    "service_completed",
    "payment_success",
 ...
 */
export async function dispatchCustomerNotifications() {
    const sb = await admin();
    const sendOfferPush = await sender();
...
export async function dispatchPartnerNotifications() {
    const sb = await admin();
    const sendOfferPush = await sender();
    const { data: rows } = await sb
        .from("partner_notifications")
...
export async function dispatchAdminAlerts() {
    ...
    const admins = await sb
        ...
        .then((res) => res.data)
        .catch(() => null);
    const adminIds = (admins ?? []).map((x) => x.user_id ?? x);
    for (const r of rows) {
