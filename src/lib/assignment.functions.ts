...
export const getMyAssignment = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // Earnings: completed × rate. Held = first 7 days' completed earnings until day 15.
    const start = new Date(a.start_date);
    const dayNum = Math.floor((Date.now() - start.getTime()) / 86400000) + 1;
    const heldDays = 7;
    const releaseDay = 15;
    const earned = completed * Number(a.rate_per_car);
    ...
    const held = heldCount * Number(a.rate_per_car);
...
export const getEndOfDaySummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // Hours: first started -> last completed
    ...
    // Distance: sum haversine between consecutive completed services
