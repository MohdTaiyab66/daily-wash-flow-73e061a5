...
export const createRazorpayOrder = createServerFn({ method: "POST" })
    ...
    if (isSubscription && booking.vehicle_id) {
        ...
        if (openSub) {
            await supabaseAdmin.from("subscription_block_log").insert({
                ...
                existing_subscription_id: openSub.id,
    ...
    const existingOrderId = booking.razorpay_order_id;
    if (existingOrderId) {
    ...
    const paymentSeed = {
        ...
        currency: "INR",
        status: "created",
        metadata: { service_type: booking.service_catalog?.service_type ?? null, razorpay_order: payload },
    };
    ...
    return { keyId, orderId: payload.id, amount: amountPaise, currency: "INR", bookingId: data.bookingId };
});
export const verifyRazorpayPayment = createServerFn({ method: "POST" })
    ...
    try {
        ...
        if (!paymentResponse.ok)
            throw new Error(payment?.error?.description || "Could not verify Razorpay payment");
        const amountPaise = Math.round(Number(booking.total_amount ?? 0) * 100);
        if (payment.status === "authorized") {
            ...
            if (!captureResponse.ok)
                throw new Error(capture?.error?.description || "Could not capture Razorpay payment");
        }
        else if (payment.status !== "captured") {
            throw new Error(`Razorpay payment is ${payment.status ?? "not captured"}`);
        }
        const { supabase } = context;
        const { data: result, error } = await supabase.rpc("activate_paid_booking", {
        ...
        try {
            await supabase.rpc("ds_on_payment_verified", { p_booking_id: data.bookingId });
        }
        catch (e) {
        ...
        try {
            await supabaseAdmin.rpc("sweep_subscription_offers");
        }
        catch (e) {
...
export const logPaymentAttempt = createServerFn({ method: "POST" })
    ...
    if (!data.attemptNo) {
        const { data: last } = await supabaseAdmin
            ...
            .limit(1)
            .maybeSingle();
        const lastNo = Number(last?.attempt_no ?? 0);
    ...
    try {
        ...
        await supabaseAdmin.rpc("ds_log_event", {
            p_booking_id: data.bookingId,
            p_stage: stage,
            p_source: "new",
            p_actor: "logPaymentAttempt",
            p_status: data.outcome === "success" ? "ok" : (data.outcome === "started" || data.outcome === "retry" ? "ok" : "error"),
            p_payload: { channel: data.channel, attempt_no: attemptNo, error_code: data.errorCode ?? null },
...
export const acquireCheckoutHold = createServerFn({ method: "POST" })
    ...
    const { data: result, error } = await supabase.rpc("acquire_checkout_hold", {
        p_booking_id: data.bookingId,
    ...
    if (error)
        throw new Error(error.message);
    return (result ?? { acquired: false, reason: "unknown" });
});
export const releaseCheckoutHold = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data, context }) => {
    const supabase = context.supabase;
    const { error } = await supabase.rpc("release_checkout_hold", {
        p_booking_id: data.bookingId,
        p_holder_id: data.holderId,
    });
    if (error)
