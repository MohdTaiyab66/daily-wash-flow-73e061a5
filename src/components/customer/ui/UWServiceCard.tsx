...
export function UWServiceCard({
  ...
  return (
    <Surface 
      className={cn("p-0 overflow-hidden flex flex-col h-[195px] bg-white transition-opacity duration-200 border-none rounded-[14px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.03)]", className)}
    >
      <div className="relative h-[112px] w-full bg-[#F8F9FB] overflow-hidden rounded-t-[14px]">
        {image ? (
          <img 
            src={image} 
            alt={name} 
      ...
      <div className="flex flex-col flex-1 p-2 pt-1.5 pb-2">
        <div className="h-[56px] flex items-center justify-center overflow-hidden px-0.5">
          <h3 className="text-[15px] font-[600] leading-[1.3] text-[#2D2D2D] line-clamp-2 text-center overflow-hidden">
            {name}
          </h3>
        </div>
