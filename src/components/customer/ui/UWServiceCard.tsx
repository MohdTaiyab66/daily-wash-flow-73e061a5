...
export function UWServiceCard({
  ...
  return (
    <Surface 
      className={cn(
        "p-0 overflow-hidden flex flex-col h-[195px] bg-white border border-[#2D2D2D]/5 rounded-[12px] shadow-[0_1px_4px_rgba(0,0,0,0.02)] transition-all active:scale-[0.96] duration-150", 

        className
      )}
    >
      <div className="relative h-[105px] w-full bg-[#F8F9FB] overflow-hidden rounded-t-[12px] shrink-0">
        {image ? (
          <img 
            src={image} 
            alt={name} 
          ...
          <div className="h-full w-full flex flex-col items-center justify-center bg-[#F8F9FA] text-[#7A7A7A]/20 p-2">
            <Car className="h-6 w-6 mb-1 opacity-10" />
            <span className="text-[8px] font-[700] uppercase tracking-[0.1em] opacity-40 text-center">Coming soon</span>
          </div>
        )}
      ...
      <div className="flex flex-col flex-1 p-2 pt-1.5 pb-2 min-w-0">
        <div className="h-[60px] flex items-center justify-center overflow-hidden px-0.5">
          <h3 className="text-[12.5px] font-[600] leading-[1.25] text-[#2D2D2D] text-center overflow-hidden break-words line-clamp-3 px-1">
            {name}
          </h3>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between gap-1 h-[36px]">
          <div className="flex items-baseline gap-1 overflow-hidden">
            ...
            {oldPrice && (
              <span className="text-[10px] font-medium text-[#7A7A7A]/40 line-through truncate">
                ₹{oldPrice}
              </span>
            )}
          </div>
