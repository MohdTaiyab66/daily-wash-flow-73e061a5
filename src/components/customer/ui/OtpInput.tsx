...
export function OtpInput({
  ...
  useEffect(() => {
    ...
    (navigator.credentials as any)
      ...
      .catch(() => { /* user dismissed or unsupported — silent by design */ });
    return () => ac.abort();
  ...
  return (
    <div className="flex items-center justify-center gap-2.5" data-testid="otp-input">
      {digits.map((d, i) => (
        <input
          key={i}
          ...
          className={cn(
            "h-14 w-12 rounded-2xl border bg-card text-center text-xl font-semibold tabular-nums outline-none transition-all",
            "focus:border-primary focus:ring-2 focus:ring-primary/25",
            d ? "border-primary/50" : "border-input",
            disabled && "opacity-60",
          )}
        />
      ))}
